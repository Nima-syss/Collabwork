...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function saveBudget(category, limit, used) {
        await apiFetch('POST', { category, limit, used });
    }

    async function removeBudget(category) {
        await apiFetch('DELETE', { category });
    }
    ...
    function openModal(mode, categoryName, budgets) {
        ...
        function syncFromSelected() {
            ...
            limitInput.value = current ? String(current.limit) : '';
    ...
    async function render() {
        ...
        const totalLimit = displayBudgets.reduce((s, b) => s + (Number(b.limit) || 0), 0);
        const totalUsed = displayBudgets.reduce((s, b) => {
            const limit = Number(b.limit) || 0;
            const used = Number(b.used) || 0;
            return s + (limit > 0 ? Math.min(used, limit) : 0);
        }, 0);
        ...
        displayBudgets.forEach(budget => {
            ...
            const hasLimit = (budget.limit || 0) > 0;
            const used = hasLimit ? Math.min(budget.used || 0, budget.limit || budget.used || 0) : 0;
            const rem = hasLimit ? Math.max(0, budget.limit - used) : 0;
            const itemPercent = hasLimit ? (used / budget.limit) * 100 : 0;
            const statusText = !hasLimit ? 'Not set' : (itemPercent >= 100 ? 'Limit reached' : 'On track');
            const limitText = hasLimit ? money(budget.limit).replace('NRP ', '') : '—';
            const remText = hasLimit ? (money(rem).replace('NRP ', '') + ' remaining') : 'Set a limit to track spending';
    ...
    form.addEventListener('submit', async function (e) {
        ...
        if (!Number.isFinite(limit) || limit < 0 || !Number.isFinite(used) || used < 0) {
            alert('Limit and used must be non-negative numbers.');
            return;
        }
